...
void setup() {
  delay(10);
  DEBUG("2.9inchb V2 e-Paper demo\r\n");
  DEV_Module_Init();
  EPD_2IN9B_V2_Init();
  EPD_2IN9B_V2_Clear();
  DEV_Delay_ms(500);
    ...
    Paint_NewImage(IMAGE_BW, EPD_2IN9B_V2_WIDTH, EPD_2IN9B_V2_HEIGHT, IMAGE_ROTATE_90, IMAGE_COLOR_POSITIVE);
    Paint_Clear(WHITE);

    //3.Read BMP pictures into RAM
    SDCard_ReadBMP("2in9cb.bmp", 0, 0);
    
    Paint_NewImage(IMAGE_BWR, EPD_2IN9B_V2_WIDTH, EPD_2IN9B_V2_HEIGHT, IMAGE_ROTATE_90, IMAGE_COLOR_POSITIVE);
    Paint_Clear(WHITE);
    SDCard_ReadBMP("2in9cy.bmp", 0, 0);
    
    //4.Refresh the picture in RAM to e-Paper
    EPD_2IN9B_V2_Display();
    DEV_Delay_ms(2000);
#elif 0
